...
export function AudienceLine({ children }: { children: React.ReactNode }) {
  return (
    ...
    >
      ...
      <p className="text-[15px] font-light text-[#aeab99] leading-relaxed max-w-xl">
        {children}
      </p>
    </motion.div>
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 mb-20 max-w-3xl">
      {beats.map((b, i) => (
        ...
        >
          <p className="text-[10px] font-mono tracking-[0.3em] uppercase text-[#7c7a6d] mb-2">
            {b.name}
          </p>
          <p className="text-[12px] text-[#868376] font-light leading-relaxed">
            {b.text}
          </p>
        </motion.div>
      ))}
...
export function MediaSlot({
  ...
  return (
    <motion.div
      className="mb-20 max-w-3xl border border-[#1c2119] bg-[#0f120e]"
